...
.precio { color: #198754; font-weight: bold; font-size: 20px; }

img { height: 220px; object-fit: contain; }
#cart-count{
...
<h2 class="text-center mb-5">Plomería</h2>
...
<h5><?= $fila['nombre'] ?></h5>
<p class="text-muted">Código: <?= $fila['id'] ?></p>
<p class="precio">RD$ <?= number_format($fila['precio'],2) ?></p>
...
onclick="agregarCarrito(<?= $fila['id'] ?>,'<?= addslashes($fila['nombre']) ?>',<?= $fila['precio'] ?>,this)">
Agregar al carrito
</button>
